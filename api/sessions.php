...
class SessionManager
{
    ...
    private function addFoodAndDrinks()
    {
        ...
        $stmt = $this->db->prepare("
            ...
            (session_id, item_name, quantity, unit_price, total_price, item_type) 
            VALUES (?, ?, ?, ?, ?, 'food_drinks')
        ");
        $stmt->bind_param("isidd", $session['id'], $item_name, $quantity, $price, $total_price);
        ...
        if ($stmt->execute()) {
            ...
            $this->logActivity("Added {$quantity}x {$item_name} to session on console {$console_id}", $console_id);
            return ['success' => true, 'message' => 'Food & Drinks added successfully'];
        } else {
            return ['success' => false, 'message' => 'Failed to add Food & Drinks'];
        }
    }
...
echo json_encode($sessionManager->handleRequest());
